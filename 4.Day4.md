...
```
Now, even if you **don't** have plink, your script will work because my docker image has PLINK insalled
...
Script runs on the **head** node
    - Nextflow uses the `executor` information to decide how
    - The job should run:
...
You need to put some information in your `nextflow.config` file, e.g., the Amazon AMI, your credentials, etc. In this example `GenomeCloud is a name I have given --- I am creating a cluster with one master and four worker nodes.
...
### 6.4. Using \texttt{join} on our example:
```nextflow
process P3 {
    echo true
