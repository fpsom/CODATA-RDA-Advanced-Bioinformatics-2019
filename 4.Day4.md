...
### Lead Instructor
- [Phelelani Mpangase](https://github.com/phelelani) |  [@pmpangase](https://twitter.com/pmpangase) | [email](mailto:Phelelani.Mpangase@wits.ac.za)
...
**Exercise 1 Solution - using `bash`:**
```bash
...
**Exercise 1 Solution - using `Nextflow`:**
```nextflow
#!/usr/bin/env nexflow
...
- [x] Parameterise the input.
- [x] Want output to go to convenient place.
- [x] Workflow takes in multiple input files -- processes are executed on each in turn.
- [x] Complication: may need to carry the base name of the input to the final output.
- [x] Can repeat some steps for different parameters.
...
The double-dash parameters are user-defined and completely extensible -- they are used to populate `params`. They modify the behaviour of *yyour* program.
...
This example takes a stream of values from \emph{params.pops} and for each value (that's what map does) it applies a closure that takes a string and produces a tuple of files. That tuple is then bound to a channel called `plink_data`. Note that there are two **distinct** uses of the `set`: 
