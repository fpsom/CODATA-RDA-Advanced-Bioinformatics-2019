...
The **slides** of the talk are available [**here**](files/slides/nextflow_tutorial.pdf).
...
**Exercise:** Have a look at [`pairs`](). In the data directory are set of data files for different years and months. First, I want to use `paste` to combine all the files for the same year and month (`paste` joins files horizontal-wise). Then these new files should be concated.
