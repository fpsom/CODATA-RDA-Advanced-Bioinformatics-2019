...
```
Using a text editor like `emacs` or `vim`, create a file named `cleanups.nf` and copy the contents of the `nextflow` script above into `cleanups.nf` file.
...
```
nextflow run cleanups.nf
```
The output we get:
```
N E X T F L O W  ~  version 19.07.0
Launching `cleanups.nf` [distraught_lamarr] - revision: fb99ce6125
executor >  local (3)
...
Done!
```
Note that `nextflow` creates a `work` directory, and inside of that are the working directories of each process -- in the example above you can see that the `getIDs` process was launched in a directory with a prefix `7e1ad1`, inside the directory `84`. The directory structure is looks like:
```
day4
|--cleanups.nf
|--data
