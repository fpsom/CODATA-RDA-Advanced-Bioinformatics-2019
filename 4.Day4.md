...
- _**09:00 - 10:00**_ [Introduction to Nextflow](4.Day4.md#-nextflow-a-tutorial-through-examples-)

- _**10:00 - 11:00**_ [Parameters, Channels and Processes](4.Day4.md#3-generalising-and-extending)

- _**11:00 - 11:30**_ _**Coffee break**_

- _**11:30 - 12:30**_ [Docker/Singularity](4.Day4.md#4-nextflow--dockersingularity), [Executors](4.Day4.md#5-executors) and [Channel Operations](4.Day4.md#6-channel-operations)

- _**12:30 - 14:00**_ _**Lunch break**_

- _**14:00 - 15:00**_ [Practical exercises in Nextflow](4.Day4.md#7-practical-execise-using-the-h3abionet-variant-calling-pipeline)

- _**15:00 - 16:00**_ [Practical exercises in Nextflow](4.Day4.md#7-practical-execise-using-the-h3abionet-variant-calling-pipeline)

- _**16:00 - 16:15**_ _**Coffee break**_

- _**16:15 - 18:00**_ [Practical exercises in Nextflow](4.Day4.md#7-practical-execise-using-the-h3abionet-variant-calling-pipeline)
...
```bash
wget https://github.com/fpsom/CODATA-RDA-Advanced-Bioinformatics-2019/blob/master/files/data/11.bim
less -S 11.bim
mv 11.bim data/
```
...
### 3.3. [Channels](https://www.nextflow.io/docs/latest/channel.html)
...
Multiple parameters:
**Exercise:** Now try adding a process to our Nextflow example and for splitting the file but using different split values, e.g.:
```bash
...
```
Have a look at the scrip [here](files/data/cleandups_channels.nf)

### 3.5. Managing Grouped Files
We've seen so far where we have a stream of file being processed independently. But in many applications there may be matched data sets. We'll now look at an example, using a popular bioinformatics tool called PLINK. In its most commn usages, PLINK takes in three related files, typically with the same but different suffixed: `.bed`, `.bim`, `.fam`.
...
params.dir = "/popdata/1k-2014/pops/"
dir = params.dir
...
This example takes a stream of values from \emph{params.pops} and for each value (that's what map does) it applies a closure that takes a string and produces a tuple of files. That tuple is then bound to a channel called `plink_data`. **NB:** There are two **distinct** uses of the `set`: 
...
Now let's look at more realistic exammple. To try this eaxmple on your own computer.
...
**Exercise:** Do \texttt{git checkout pairs}. In the data directory are set of data files for different years and months. First, I want to use `paste` to combine all the files for the same year and month (paste joins files horizontal-wise). Then these new files should be concated.
...
You create Docker and/or Singularity images locally, or get from repositories ():
...
singularity pull quay.io/banshee1221/h3agwas-plink
```
Creating Docker images locally
```bash
```
Creating Singularity images locally
```bash
...
## For Singularity
nextflow run plink.nf -with-singularity
```
