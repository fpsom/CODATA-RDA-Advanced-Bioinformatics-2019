...
**Exercise 4:** Now try adding a process to our Nextflow example and for splitting the file but using different split values, e.g.:
```bash
...
**Exercise 5:** Have a look at [`weather.nf`](files/data/weather.nf). In the data directory are set of data files for different years and months. First, I want to use `paste` to combine all the files for the same year and month (`paste` joins files horizontal-wise). Then these new files should be concated.
